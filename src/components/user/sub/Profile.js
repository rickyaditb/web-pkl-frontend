...
export default function Profile(props) {
    ...
    return (
        <motion.div initial={{ opacity: 0, scale: 1 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3 }} className={`col-span-12 md:col-span-4 lg:col-span-3 ${props.kelas}`}>
            ...
            <AnimatePresence>
                {dropdown &&
                <motion.div initial={{ opacity: 0, scale: 1.1 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3 }} exit={{ opacity: 0, scale: 0.9 }} className={`bg-white p-4 rounded-lg shadow mt-3 flex flex-col gap-3`}>
